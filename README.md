...
This app writes exactly one registry key, and it has done since 1.5.2: `HKCU\Software\Classes\desksideradio`, the handler behind the **Start with Windows** switch in Settings. Nothing else writes one — every `reg.exe` in the repository is a *query*, asking Windows where a browser was installed.
