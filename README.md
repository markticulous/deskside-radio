...
To take the update, double-click `Win-Install-or-Update-Deskside-Radio.cmd` in the app folder — hold Shift on **Settings → Service** for a button that opens that folder — or run **Deskside Radio - Update** from the Start menu. The file is named first because it is the one that is always there: it ships inside the zip, so it is beside `index.html` however the folder arrived, while the Start menu entry exists only where the installer created it. It is the same file that installed it, and it works out that it is being run from inside an install and updates that folder in place.
